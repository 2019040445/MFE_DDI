...
def drug_embedding(id):
    ...
    # print('sdf2graph -----------------------------')
    # print(type(x), type(edge_attr), type(edge_index))
    # print(x.shape, edge_attr.shape, edge_index.shape)

    x, edge_attr, edge_index = smile_to_enhanced_graph(id)
    # print('sdf2graph -----------------------------')
    # print(type(x), type(edge_attr), type(edge_index))
...
class Dataset(data.Dataset):
    ...
    def __init__(self, list_IDs, labels, df_dti): # 加载所有数据
        'Initialization'
        ...
        self.smiles1 = self.df["smile2"].values
        self.smiles2 = self.df["smile1"].values
    ...
    def __getitem__(self, index):
        ...
        d_node, d_attn_bias, d_spatial_pos, d_in_degree, d_out_degree, d_edge_input = drug_embedding(self.smiles1[index])#molormer drug1_id
        p_node, p_attn_bias, p_spatial_pos, p_in_degree, p_out_degree, p_edge_input = drug_embedding(self.smiles2[index])
...
def drug2emb_encoder(x):
    ...
    vocab_path = './ESPF/drug_codes_chembl.txt'  # 存储了通过 Byte Pair Encoding (BPE) 算法生成的子词合并规则（类似NLP中的分词词典）。
    bpe_codes_drug = codecs.open(vocab_path)
    dbpe = BPE(bpe_codes_drug, merges=-1, separator='') # 将药物字符串拆分为子词
    sub_csv = pd.read_csv('./ESPF/subword_units_map_chembl.csv')
    idx2word_d = sub_csv['index'].values
    words2idx_d = dict(zip(idx2word_d, range(0, len(idx2word_d))))
    max_d = 50
    t1 = dbpe.process_line(x).split() ## 拆分SMILES为子词列表
    try:
