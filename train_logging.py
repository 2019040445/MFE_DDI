...
def evaluate_net(net, train_dataloader, validation_dataloader, test_dataloader, criterion, args):
    ...
    new_best_model_found = test_scores[0] > g.best_mean_test_score
    # F1,accuracy,recall,precision,auroc

    if new_best_model_found:
        # g.best_mean_train_score = train_scores[4]
        g.best_mean_validation_score = validation_scores[0]
        g.best_mean_test_score = test_scores[0]

        if args.savemodel:
            # path = SAVEDMODELS_DIR + type(net).__name__
            path = os.path.join(SAVEDMODELS_DIR, f"{g.best_mean_test_score}_full.pth")
            print(path)
            torch.save(net, path)
